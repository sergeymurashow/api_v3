...
export default class FormatReport {
	...
	async result(bookings = this.bookings) {
		...
		// TODO: Make booking checkout for containers. If containers exist for booking patch contacts into containers
		...
		const formattedReport = await Promise.all(bookings.map(async (b, i, a) => {
			...
			if (this.patch[b.bookingId]) {
				return {
					...
					method: 'patch',
					recordId: this.patch[b.bookingId].id
				}
			}
				...
				return {
					data: {
						...
						120: b.mark, // mark
						130: this.cache.get(b.contract).map( m => ({ recordId: m.recordId, catalogId: m.catalogId })),
